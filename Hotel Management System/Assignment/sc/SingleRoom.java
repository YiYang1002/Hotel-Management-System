    ...
    public void RoomMenu() {
        ...
        while (true) {
            try {
                ...
                String query = "SELECT * FROM room WHERE Room_Type = 'Single'";
                ...
                System.out.printf("%7s %98s", "| |", "| |\n");
                // for loop is usually used when you know exactly how many times to iterate
                /* rs.next() works like a cursor that starts before the first row of the query result.
                   rs.next() returns true if there is a next row and moves to it.
                   rs.next() returns false if there are no more rows. */

                while (rs.next()) {
                    a++;
                    System.out.printf("%7s %6s %9s %13s %17.2f %21s %15s %11s", "| |", "[" + a + "]", 
